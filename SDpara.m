...
b=0:0.01:3; c=1;
for i=1:length(b)
    Payoff_SD=[(b(i)-c)/2 b(i)/2-c; b(i)/2 0];
    dxdt =@(t,x) [x(1)*((b(i)-c)/2*x(1)*(1-x(1))+(c-b(i))*x(1)*x(2)+(b(i)/2-c)*x(2)); x(2)*(b(i)/2*x(1)*(1-x(1))+c/2*x(1)^2+(c-b(i))*x(1)*x(2))];
...
rng('default')
% parameters
b=0:0.01:3; c=1; h=2/sqrt(10); dt=(h^2)/5;
        ...
        for j=2:m-1
            for k=2:l-1
                ...
                C(j,k,i+1)=C(j,k,i)+dt*((C(j+1,k,i)+C(j-1,k,i)+C(j,k+1,i)+C(j,k-1,i)-4*C(j,k,i))/h^2)+...
                    dt*C(j,k,i)*((b(z)-c)/2*C(j,k,i)*(1-C(j,k,i))+(c-b(z))*C(j,k,i)*D(j,k,i)+(b(z)/2-c)*D(j,k,i));
                % Defectors
                D(j,k,i+1)=D(j,k,i)+dt*((D(j+1,k,i)+D(j-1,k,i)+D(j,k+1,i)+D(j,k-1,i)-4*D(j,k,i))/h^2)+...
                    dt*D(j,k,i)*(b(z)/2*C(j,k,i)*(1-C(j,k,i))+c/2*C(j,k,i)^2+(c-b(z))*C(j,k,i)*D(j,k,i));
            end
        end
    end
...
B=0:0.01:3; c=1;
for z=1:length(B)
    Payoff=[(B(z)-c)/2 B(z)/2-c; B(z)/2 0];
    % Allocations
    pC = zeros(max_iter,1);
    Payment = zeros(N,N);
    NE = zeros(N,N);
