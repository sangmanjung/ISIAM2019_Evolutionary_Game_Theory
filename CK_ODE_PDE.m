...
close, clear, clc
a=0.2; b=1.9; Payoff=[0 -a; a -b]
dxdt =@(t,x) [x(1)*x(2)*(b*x(2)-a); x(2)*(a*x(1)+b*x(2)*(x(2)-1))];
[t,y]=ode45(dxdt,[0 100],[0.9 0.1]);
% figure(1)
subplot(1,2,1)
plot(t,y(:,1),'g',t,y(:,2),'b','LineWidth',2);grid
legend('Cooperators','Defectors')
title('Generalized Chicken games in ODE')
xlabel('Time'); ylabel('C and D');
%% Replicator Dynamics : PDE
clear, clc
rng('default')
% parameters
a=0.2; b=1.3; h=2/sqrt(10); dt=(h^2)/5;
...
title('Generalized Chicken games in PDE'); xlabel('Time'); ylabel('C and D');
% ylim([0 1])
